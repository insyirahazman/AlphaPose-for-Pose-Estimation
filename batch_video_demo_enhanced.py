...
def check_already_processed(video_path: str, output_dir: str) -> bool:
    ...
    video_name = Path(video_path).stem
    expected_output = os.path.join(output_dir, f'AlphaPose_{video_name}.avi')
    expected_json = os.path.join(output_dir, f'{video_name}.json')
    
    return os.path.exists(expected_output) and os.path.exists(expected_json)
